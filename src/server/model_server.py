...
from util.loader import parse_filters, is_valid_model, load_json, load_weight, get_model_group_path, get_archived_file, METADATA_FILENAME, CHECKPOINT_FOLDERNAME, get_pipeline_path, any_node_type, is_matched_type, get_largest_candidates, default_pipelines, get_node_type_from_name
...
def select_best_model(spec, valid_grouppath, filters, energy_source, pipeline_name="", trainer_name="", node_type=any_node_type, weight=False, loose_node_type=True):
    ...
    if node_type != any_node_type:
        ...
        if len(model_names) == 0:
            if not loose_node_type:
                return None, None
            print(f"{valid_grouppath} has no matched model for node type={node_type}, try all available models")
    ...
    for model_name in candidates:
        ...
        if metadata is None or not is_valid_model(metadata, filters) or ERROR_KEY not in metadata:
            # invalid metadata
            print(f"skip invalid {model_name} candidate - model validity: {is_valid_model(metadata, filters)} metadata: {metadata}")
            continue
        if weight:
...
def get_model():
    ...
    print(f"response: model {best_model['model_name']} by {best_model['features']} with {ERROR_KEY}={best_model[ERROR_KEY]} selected with uncertainty={best_uncertainty}, looseness={best_looseness}")
...
def get_available_models():
    ...
    try:
        ...
        for output_type in output_types:
            print(f"Searching output type {output_type}")
            model_names[output_type.name] = dict()
            for fg in valid_fgs:
                print(f"Searching feature group {fg}")
